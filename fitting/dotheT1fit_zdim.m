...
% no display output
opt = optimoptions('lsqcurvefit','SpecifyObjectiveGradient',true,'Diagnostics','off','Display','off','MaxIterations',100);
...
parfor k=1:dimz
    ...
    if mask(k) == 1
        ...
        % do the fit
        x = lsqcurvefit(fitfun,x0,fa,ydata,[0 0],[32767 4000],opt);
